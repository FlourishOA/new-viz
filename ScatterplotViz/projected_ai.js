...
d3.csv("recent_est_ai20180130.csv", function(error, data) {
//read in the data stored in same folder
	...
	data.forEach(function(d) {  // assign data types to each column
		...
		d.max_projaidate = +d.max_projaidate;
		d.issn = String(d.issn);
		d.proj_ai = +d.proj_ai;
...
function drawVis(dataset) { //draw the circles initially and on each interaction with a control
	...
	circle
    	  ...
    	  .attr("cy", function(d) { return y(d.proj_ai);  }) // y value based on proj_ai
    	  ...
    	  .style("fill", "DarkSlateBlue")
     	  .on("mouseover", function(d) { // set the tooltip on mousehover
				...
				tooltip.transition()
					.duration(200)
					.style("opacity", .9)
				tooltip.html("Journal: " + "<a href=http://flourishoa.org/journal/" + d.issn + "/ target='_blank'>" + d.journal_name + "</a>"
				+ "<br>Category: " + d.category
				+ "<br>APC: $" + d.price 
				+ "<br>APC Date: " + formatDate(d.max_apcdate)
				+ "<br>AI Score: " + d.proj_ai.toFixed(2)
                + "<br>AI Score Date: " + formatDate(d.proj))
					.style("left", (d3.event.pageX + 5) + "px")
					.style("top", (d3.event.pageY - 28) + "px");
				function drawQuad() {
				var scaledX = x(d.price);
				var scaledY = y(d.proj_ai);
			        svg.append("rect")
                            .attr("id", "better")
                            .attr("x", 0)
                            .attr("y", 0)
	...
	circle.enter().append("circle")
    	  ...
    	  .attr("cy", function(d) { return y(d.proj_ai);  })
    	  .attr("r", 4.5)
		  .style("stroke-width", 2)
    	  .style("stroke", "black")
    	  .style("fill", "DarkSlateBlue")
